sbAdminApp.directive('clsSelect', function ($rootScope, $log, $state) {
    return {
        restrict: 'C',
        require: 'ngModel',
        link: function (scope, elem, attrs, ngModel) {
             $(".cls_select ul").niceScroll({styler: "fb", cursorcolor: "#f05826", cursorwidth: '6', cursorborderradius: '10px', background: '#f05826', spacebarenabled: false, cursorborder: '', zindex: '1000'});
              scope.$watch("searchObj.pick_up_time", (function (newValue, oldValue) {
                //element.datepicker("option", "maxDate", newValue);
                console.log(newValue);
               scope.searchObj.drop_off_time = newValue;
            }), true);
            elem.bind('click .placeholder', function (e) {
               // console.log(e);
               // console.log(ngModel);
                var parent = $(this).closest('.cls_select');
                if (!parent.hasClass('is-open')) {
                    parent.addClass('is-open');
                    $('.cls_select.is-open').not(parent).removeClass('is-open');
                    console.log('open');
                } else {
                    //parent.find('.placeholder').text();
                    parent.removeClass('is-open');
                    console.log('close');
                    var selected_value = parent.find('.placeholder').attr("custom_value");
                    console.log("selected_value : " + selected_value);
                    ngModel.$setViewValue(selected_value);
                }
                e.preventDefault();
            });
        }
...
sbAdminApp.directive('clsSelectLi', function ($rootScope, $log, $state) {
    return {
        restrict: 'C',
        link: function (scope, elem, attrs) {
            elem.on('click', function (e) {
               ...
               // console.log(attrs["customValue"]);
               // console.log($(this).text());
                parent.find('.placeholder').text($(this).text());
                parent.find('.placeholder').attr("custom_value", attrs["customValue"]);
                e.preventDefault();
            });
        }
    };
